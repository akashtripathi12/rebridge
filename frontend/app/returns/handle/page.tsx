...
/**
 * Operator-only Return Product page.
 * Full desktop layout for processing incoming returns — photograph, grade,
 * and route each item. Operator fills in order context manually.
 * Customers see a role-gate card.
 */
export default function ReturnHandlePage() {
  ...
  // --- Operator-entered context fields (no built-in values) ---
  ...
  const grade = useCallback(async () => {
    try {
      ...
      if (shots[0])
        sessionStorage.setItem(`rb:photo:${meta.item_id}`, shots[0].url);
  ...
  }, [shots, simBlurry, router, category, ageMonths, orderId]);
  ...
  return (
    <main className="bg-canvas px-4 py-10 sm:px-6">
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.05fr_minmax(0,400px)]">
          {/* Left — Photo capture area */}
            ...
            >
              <div className="relative z-10 flex flex-col items-center gap-4 text-center text-white">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-pill px-5 py-2.5 font-sans text-[13px] font-bold transition-transform ${
                    remaining > 0
            ...
            <div className="grid grid-cols-4 gap-2.5" data-testid="batch-strip">
              {Array.from({ length: 4 }).map((_, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`group relative flex h-20 items-center justify-center overflow-hidden rounded-card border bg-paper ${
                      s
                        ? "border-hair"
                        : "border-dashed border-hair text-[11px] text-stone"
                    }`}
                  >
                    {s ? (
                      <>
          ...
          <aside className="flex flex-col gap-5">
            <div className="rounded-card border border-hair bg-pearl p-5 shadow-sm">
              ...
              <p className="mt-2 text-[12px] text-mute">
                Enter the return order details. Helps the AI grade accurately
                and route to the best buyer.
              </p>
            ...
            <div className="rounded-card border border-hair bg-paper p-5 shadow-sm">
              ...
              <ul className="mt-3 space-y-2.5 text-[12.5px] text-ash">
                <li className="flex items-start gap-2">
                  ...
                  Add 2–4 photos of the returned item
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-[5px] h-[7px] w-[7px] flex-none rounded-full bg-amber-deep" />
                  AI grades condition, defects &amp; completeness
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-[5px] h-[7px] w-[7px] flex-none rounded-full bg-amber-deep" />
                  Auto-routes to resale, refurb, or donation
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-[5px] h-[7px] w-[7px] flex-none rounded-full bg-trust" />
                  Health Card generated for the buyer
                </li>
              </ul>
            </div>
            ...
            {itemsService.mode === "mock" && (
              <label className="flex items-start gap-2 rounded-card border border-dashed border-hair bg-paper px-3 py-2.5 text-[11.5px] text-mute">
                ...
                <span>
                  ...
                  simulate a blurry capture so the AI requests a retake.
                </span>
              </label>
            )}
            ...
            <div className="flex flex-col gap-2">
              <Button
                ...
                onClick={grade}
              >
                <Camera className="h-4 w-4" /> Grade return{" "}
                {shots.length > 0 ? (
              ...
              {!orderId.trim() && shots.length >= 2 && (
                <p className="text-center text-[11px] text-sale">
                  Enter an Order ID to proceed.
                </p>
              )}
              <p className="text-center text-[11px] text-mute">
                Takes ~5s. Photos go straight to S3 — never through us in
                plaintext.
              </p>
            </div>
          </aside>
...
/** Shared header for the operator return page. */
function PageHeader() {
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <div className="font-sans text-[12px] font-bold uppercase tracking-[0.2em] text-amber-deep">
          Operator · Return Product
        </div>
        <h1 className="mt-1 font-display text-[clamp(28px,4vw,40px)] font-extrabold uppercase leading-none tracking-tight">
          Handle a return
        </h1>
        <p className="mt-3 max-w-[60ch] text-[14px] leading-relaxed text-ash">
          Photograph the returned product, fill in the order context, and let
          the AI grade it. The item will be routed to resale, refurb, or
          donation based on the grade.
        </p>
      </div>
    </div>
  );
