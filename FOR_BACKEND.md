...
# Flow-wiring pass — one new note (not an endpoint)

The demo now threads ONE item through the whole journey via a deterministic id
(`itm_demo`, currently the seeded `itm_shoe7`). In **mock** mode the frontend
forces that id on `createItem` so "the item I graded" == "the marketplace top
match" == "the Health Card subject", and the matching engine pins it.

**In LIVE mode** `createItem` returns a backend-assigned id (the optional
`item_id` hint is ignored, as intended). For the golden path to stay "one item"
on live data, the backend needs to: (a) let that graded item, once **listed**,
appear in `GET /marketplace`, and (b) have `GET /items/{id}/matches` return it as
a near buyer so it surfaces as the routed match. No new endpoint — just confirms
the item flows listing → marketplace → matches under its real id. Until then the
connected demo runs on mocks (all flags default false), exactly as the other
phases.
